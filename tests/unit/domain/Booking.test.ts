...
function make(id: string, start: string, end: string): Booking {
  return new Booking(id, "room-1", "user-1", "Reunião", new Date(start), new Date(end));
}

describe("Booking.conflictsWith", () => {
  it("retorna true quando há sobreposição de horário", () => {
